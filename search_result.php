...
<html>
	...
	<div class='container-fluid' style='margin-top:2%;margin-bottom:4%'>
		<?php
			...
			$to=$_POST["dstn"];
			$type=$_POST["type"];
			$date= isset($_POST["journey_date"]) ? $_POST["journey_date"] : date('Y-m-d'); 
			$i=1;
						...
						while($data=mysqli_fetch_array($records))
						{
							...
							echo "<tr style='height:120px'>";
								...
								echo "<td style='list-style:none;text-align:middle;'class='text-muted'>
										<h2 class='btn bus_detail_info' data-toggle='modal' data-target='#ModalforBusDetail' id='businfo_".$id."' style='padding:0;color:#333333;font-size:20px;font-weight:bold;text-decoration:underline;'>
											".$data['bus_company']."
										</h2>
										<li>Coach#".$data['bus_no']."</li>
										<li><small>Cancellation policy</small></li>
									  </td>";
								echo "<td style='list-style:none;font-size:20px;'class='text-muted'>
										<h5 style='padding:0;color:#333333;font-size:20px;font-weight:bold;'>".$data['bus_type']."</h5>
										<li>Class: Bussiness</li>
									  </td>";
								echo "<td style='list-style:none;'>
										<h5>".$data['time']."</h5>
										<li class='text-muted'><small>Depart from: $from</small></li>
										<li class='text-muted'><small>Last stopage: $to</small></li>
									 </td>";
								echo "<td>
										<h5 class='text-muted' style='margin-top:20px;'><small>Total seats: ".$data["seats"]." </small></h5>
									 </td>";
								echo "<td style='list-style:none;text-align:middle;' class='text-muted'>
										<h5 style='color:#079d49;font-weight:bold;'> INR ".$data['fare']."/-</h5>
										<li>(Per seat)</li>
